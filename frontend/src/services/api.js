...
export async function fetchCsrfToken() {
  try {
    ...
    console.error('CSRF token request failed:', res.status, res.statusText);
  } catch (err) {
...
async function request(url, options = {}, isRetry = false) {
  ...
  // If CSRF token rejected (403), refresh token and retry
  if (
    response.status === 403 &&
    !isRetry &&
  ...
  if (!response.ok) {
    ...
    if (!message) {
      if (
        ...
        message =
          'Backend server is unreachable. Please verify that the backend is running.';
      } else {
